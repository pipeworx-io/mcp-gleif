...
```bash
curl -X POST https://gateway.pipeworx.io/v1/tools/search_lei \
  -H 'Content-Type: application/json' \
  -d '{"query":"Apple Inc"}'
```

No account needed for the first calls. Inspect any tool: `GET https://gateway.pipeworx.io/v1/tools/search_lei`. Find one: `POST https://gateway.pipeworx.io/v1/tools/search_packs` with `{"query":"..."}`.
